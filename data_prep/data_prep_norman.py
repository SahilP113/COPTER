...
def get_train_data(adata, adata_control, splits_data):
    ...
    for perturbation in splits_data["train"]:
        if perturbation != "ctrl":
            ...
            for i in range(perturb_data.n_obs):
                train_genes.append(perturbation)
...
def get_val_data(adata, adata_control, splits_data):
...
def get_test_data(adata, adata_control, splits_data):
    ...
    for perturbation in splits_data["test"]:
        if perturbation != "ctrl":
            ...
            perturb_data = adata[adata.obs["condition"] == perturbation]
            for i in range(perturb_data.n_obs):
                test_genes.append(perturbation)
                rand_int = np.random.randint(0, adata_control.n_obs)
                random_control = torch.tensor(control_array[rand_int])
...
def get_all_data(data_file_path, gene_ens_path, ensembl_aa_path, splits_file_path):
    ...
    train_genes, train_controls, train_y, train_de_idxs = get_train_data(adata_perturb, adata_control, splits_data)
    val_genes, val_controls, val_y, val_de_idxs = get_val_data(adata_perturb, adata_control, splits_data)
    test_genes, test_controls, test_y, test_de_idxs = get_test_data(adata_perturb, adata_control, splits_data)
