...
from train_utils_RN_NA import training_and_validation
...
def run_finetune(models_output_dir, plm_type, task_name, embed_name, embed_name2, hparams, batch_size, num_epoch, train_size, val_size, weigh_sample, weigh_loss):
    ...
    for i in range(num_folds):
        ...
        data_folder = "data/tc-hard/ds.hard-splits/pep+cdr3b+cdr3a+MHC"

        if task_name == "RN":
            task_folder = "only-sampled-negs"
        elif task_name == "NA":
            task_folder = "only-neg-assays"
        else: 
            raise ValueError(f"Unknown task name: {task_name}")

        train_path = os.path.join(data_folder, "train", task_folder, "new_train-" + str(i) + ".csv")
        val_path = os.path.join(data_folder, "train", task_folder, "val-" + str(i) + ".csv")
        test_path = os.path.join(data_folder, "test", task_folder, "test-" + str(i) + ".csv")
        ...
        for row in range(train_data.shape[0]):
            TCR = train_data.iloc[row]["cdr3.alpha"] + "X" + train_data.iloc[row]["cdr3.beta"]
            TCR_train.append(TCR)
        #TCR_train = list(train_data["cdr3.beta"])
        epitope_train = list(train_data["antigen.epitope"])
        label_train = torch.tensor(train_data["label"])

        TCR_val = []
        for row in range(val_data.shape[0]):
            TCR = val_data.iloc[row]["cdr3.alpha"] + "X" + val_data.iloc[row]["cdr3.beta"]
            TCR_val.append(TCR)
        #TCR_val = list(val_data["cdr3.beta"])
        epitope_val = list(val_data["antigen.epitope"])
        ...
        for row in range(test_data.shape[0]):
            TCR = test_data.iloc[row]["cdr3.alpha"] + "X" + test_data.iloc[row]["cdr3.beta"]
            TCR_test2.append(TCR)
        #TCR_test2 = list(test_data["cdr3.beta"])
        epitopes_test2 = list(test_data["antigen.epitope"])
        labels_test2 = torch.tensor(test_data["label"])


        unique_epitopes = np.unique(test_data["antigen.epitope"])
        ...
        for epitope in unique_epitopes:
            ...
            for row in range(test_data.shape[0]):
                if test_data.iloc[row]["antigen.epitope"] == epitope:
                    TCR = test_data.iloc[row]["cdr3.alpha"] + "X" + test_data.iloc[row]["cdr3.beta"]
                    #TCR = test_data.iloc[row]["cdr3.beta"]
        ...
        #Positive_proportion_train and positive_proportion_test may not be numbers, cannot do np.mean()
...
def get_padded_cdr(cdr_batch, batch_max, plm_type):

    data_folder = "data/tc-hard/ds.hard-splits/pep+cdr3b+cdr3a+MHC"

    if plm_type == "esm2_8m":
        embeddings_file = "ESM_esm2_t6_8M_UR50D_features_cdr.h5"
    elif plm_type == "esm2_650m":
        embeddings_file = "ESM_esm2_t33_650M_UR50D_features_cdr.h5"
    elif plm_type == "progen2_small":
        embeddings_file = "ProGen_progen2-small_features_cdr.h5"
    elif plm_type == "progen2_large":
        embeddings_file = "ProGen_progen2-large_features_cdr.h5"
    else:
        raise ValueError(f"Unknown plm_type: {plm_type}")
    
    cdr_tokenized_file = os.path.join(data_folder, embeddings_file)
    cdr_h5file = h5py.File(cdr_tokenized_file, 'r')

    cdr_embeddings = []
    for cdr_sequence in cdr_batch:
        if cdr_sequence not in cdr_h5file.keys():
            print("Missing Sequence: ", cdr_sequence)
            continue
        cdr_h5dataset = cdr_h5file[cdr_sequence]  
        esm_embed = torch.tensor(np.array(cdr_h5dataset))
        padded_embed = torch.nn.functional.pad(esm_embed, (0, 0, 0, batch_max - esm_embed.shape[0]), value=FOLDSEEK_MISSING_IDX)
        cdr_embeddings.append(padded_embed)
    cdr_embed_tensor = torch.stack(cdr_embeddings)
    return cdr_embed_tensor

def get_max_length(cdr_batch):
    batch_max_length = 0
    for cdr_sequence in cdr_batch:
...
def finetune_evaluate(unique_epitopes, clf, pooling_clf, TCR_test, epitope_test, label_test, TCR_test2, epitope_test2, label_test2, plm_type, models_output_dir, embed_name):
    ...
    with torch.no_grad():
        ...
        new_cdr_test = get_padded_cdr(TCR_test2, test_max, plm_type)  #If the initial data contains the gene names
        mask = (new_cdr_test != FOLDSEEK_MISSING_IDX)[:, :, 0]
        epitope = get_epitope(epitope_test2, plm_type)
        pooled_cdr_test = pooling_clf(new_cdr_test, epitope, mask)
        y_pred_total = torch.sigmoid(clf(pooled_cdr_test)).squeeze(-1).numpy()

    #num_positives = label_test2.sum()
    #print("Number of true positives: ", num_positives)
    ...
    #for celltype in celltype_protein_dict:
        ...
        with torch.no_grad():
            ...
            new_cdr_test = get_padded_cdr(TCR_test[epitope_type], test_max, plm_type)  #If the initial data contains the gene names
            mask = (new_cdr_test != FOLDSEEK_MISSING_IDX)[:, :, 0]
            epitope = get_epitope(epitope_test[epitope_type], plm_type)
            pooled_cdr_test = pooling_clf(new_cdr_test, epitope, mask)
            y_test_pred = torch.sigmoid(clf(pooled_cdr_test)).squeeze(-1).numpy()
...
def main(args, hparams, wandb):
    ...
    mean_stats, std_dev_stats, positive_proportion_train, positive_proportion_test, accuracy, f1_score, apr_5, apr_10, apr_15, apr_20, auroc_scores, ap_scores, bal_acc, test_recall_5, test_precision_5, test_ap_5, test_recall_10, test_precision_10, test_ap_10 = run_finetune(models_output_dir, args.plm, args.tcr_epitope_task, args.embed, args.embed2, hparams, args.batch_size, args.num_epoch, args.train_size, args.val_size, args.weigh_sample, args.weigh_loss)
