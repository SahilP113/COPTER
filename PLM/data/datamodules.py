...
class BinaryDataset(Dataset):
    ...
    def __getitem__(self, i: int):
        #drug = self.drug_featurizer(self.drugs.iloc[i])
        drug = torch.ones(2048)
        target = self.target_featurizer(self.targets.iloc[i]) #This might be the ESM embedding
        #print(target)
        label = torch.tensor(0)
        #label = torch.zeros(len(self.targets.iloc[i]))
        #label = torch.tensor(self.labels.iloc[i])
        return drug, target, label
        #drug = torch.zeros(1, 1)
        #return drug, target, 0
        #drug should be a tensor, not a string
...
class DTIDataModule(pl.LightningDataModule):
    def __init__(
        ...
        self._data_dir = Path("datasets/GenePerturb") #datasets/EpitopeTCR #Path(data_dir)
        self._train_path = Path("perturbed_genes_norman.csv") #Path("ds.csv") #Path("ds2.csv") #titan_alldata.csv
        self._val_path = Path("val.csv") #titan_val.csv            
        self._test_path = Path("test.csv") #titan_test.csv

        #self._data_dir = Path("datasets/tc-hard") #datasets/EpitopeTCR #Path(data_dir)
        #self._train_path = Path("ds2.csv") #Path("ds.csv") #titan_alldata.csv
        #self._val_path = Path("ds.hard-splits/pep+cdr3b/test/only-sampled-negs/test-0.csv") #titan_val.csv
        #self._test_path = Path("ds.hard-splits/pep+cdr3b/test/only-sampled-negs/test-1.csv") #titan_test.csv

        #self._data_dir = Path("datasets/PanPrep") #datasets/EpitopeTCR #Path(data_dir)
        #self._train_path = Path("panprep_alldata.csv") #titan_alldata.csv
        #self._val_path = Path("panprep_val.csv") #titan_val.csv
        #self._test_path = Path("panprep_test.csv") #titan_test.csv

        #self._data_dir = Path("datasets/AminoAcids") #datasets/EpitopeTCR #Path(data_dir)
        #self._train_path = Path("aminoacids2.csv") #titan_alldata.csv
        #self._val_path = Path("val.csv") #titan_val.csv
        #self._test_path = Path("test.csv") #titan_test.csv
        ...
        self._target_column = "aa_sequence" #"binding_TCR" #"peptide" #"TCR" #"antigen.epitope" #"AA Sequence" #"cdr3.beta" #"peptide"  #mhc.seq  
        #self._label_column = "Label"
        #self._dummy_column = "cdr3.beta"
    ...
    def prepare_data(self):
        ...
        all_targets = df_train[self._target_column].unique()
    ...
    def setup(self, stage: T.Optional[str] = None):
        ...
        all_targets = self.df_train[self._target_column].unique()
        ...
        if stage == "fit" or stage is None:
            ...
            self.data_val = BinaryDataset(
                ...
                #self.df_val[self._dummy_column],
        ...
        if stage == "test" or stage is None:
            self.data_test = BinaryDataset(
                ...
                #self.df_test[self._dummy_column],
...
class SCLDataModule(pl.LightningDataModule):
    def __init__(
        ...
        self._data_dir = Path("datasets/AminoAcids")
        self._all_path = Path("aminoacids2.csv")

        #self._target_column = "sequence"
        self._target_column = "AA Sequence"
