...
class SWE_Pooling(nn.Module):
    def __init__(self, d_in, num_slices, num_ref_points, freeze_swe=False):
        ...
        self.context_size = num_slices
        self.proj = nn.Linear(self.context_size, num_slices*num_ref_points, bias=False)
    ...
    #modify forward to take cell-context as parameter, m=100, L=d=320, ref=mxL, theta = dxL, w=mx1
    #Start with cell embedding modifying reference. Keep theta and w cell-agnostic. The cell embedding should be mxL.
    #Add a neural network layer that goes from 64 to mxL 
    #Try just changing Rslices
    #Set L=64
    #Use the outputs of this SWE model to do classification based on PINNACLE code
    #Feed the SWE pooling model into the classification code --> modify PINNACLE classification code
    #Load the ESM embeddings also
    def forward(self, X, C, mask=None):
        #cell context: C = Bxd where d=64
        #self.ref = self.reference.proj(C).reshape(B, M, L)
        #self.reference.proj(C) = B x mL
        '''
        ...
        #self.reference = C     #Might not even need self.reference
        M = self.num_ref_points
        ...
        #If using Rslices, then self.reference
        #Figure out what to do with self.reference, if anything
        #proj_C = proj(C)    #goes from (B, 64) to (B, 32000)
        #Rslices = proj_C.reshape(B, M, L)   #reshape from (B, 32000) to (B, 100, 32000)
        #Rslices = self.reference.proj(C).reshape(B, M, L)

        new_C = self.proj(C).reshape(M, L)    #Projects 64-dim vector to 32000-dim vector. Reshape 32000-dim vector to (1, 100, 320)
        Rslices = new_C.expand(B, M, L)  #Expands (1, 100, 320) to (B, 100, 320)
...
class DTIPooling(nn.Module):
    def __init__(
        ...
        classify=True,
        pooling="avg",
        num_ref_points=100, #1,
        freeze_swe=False,
        num_slices=64 #-1, # if this is set to -1 (default), the number of slices will be automatically set to the PLM output embedding dimensionality
    ):
        super().__init__()
    ...
    def target_pooling(self, target):
        ...
        if self.pooling_operation == 'avg':
            ...
            aggregated_target = self.pooling(target, context, mask)    #Added a context argument to account for the cell-context when pooling
            return aggregated_target
