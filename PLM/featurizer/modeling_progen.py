...
def fixed_pos_embedding(x, seq_dim=1, seq_len=None):
    ...
    # Ensure repeat_interleave will work even if sequence length is 1
    if sin.shape[0] == 0:
        sin = sin.new_zeros((1, sin.shape[1]))
        cos = cos.new_zeros((1, cos.shape[1]))
...
def apply_rotary_pos_emb(x, sincos, offset=0):
    ...
    # If rotary dim is zero, do nothing
    if sin.shape[-1] == 0:
        return x

    seq_len = x.shape[1]
    max_rotary = sin.shape[0]

    if offset + seq_len > max_rotary:
        seq_len = max_rotary - offset
        x = x[:, :seq_len]

    sin = sin[offset:offset + seq_len][None, :, None, :]
    cos = cos[offset:offset + seq_len][None, :, None, :]

    # SAFETY: guard again after slicing
    if sin.shape[-1] == 0:
        return x
