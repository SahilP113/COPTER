...
def training_and_validation(TCR_train, TCR_val, epitope_train, epitope_val, label_train, label_val, plm_type, num_epoch, batch_size, weigh_sample, weigh_loss, hparams, no_val=False):
    ...
    for i in range(num_epoch):
        print(f"Epoch {i+1}\n---------------")
        #Need to pass GNN into the train_epoch and validate_epoch methods as well
...
def train_epoch(model, pooling, train_loader, plm_type, optim, loss_func, batch_size, wandb, device):
    ...
    for i, (X, context, y) in enumerate(train_loader):
        ...
        padded_cdr = get_padded_cdr(X, batch_max, plm_type)
        epitope = get_epitope(context, plm_type)

        padded_cdr, epitope, y = padded_cdr.to(device), epitope.to(device), y.to(device)
        optim.zero_grad()

        size_batch = len(X)
        #print("Batch size: ", len(X))
        x_mask = torch.zeros((size_batch, batch_max), dtype=torch.bool)
        ...
        padding_mask = (padded_cdr != FOLDSEEK_MISSING_IDX)[:, :, 0]

        combined_mask = padding_mask & ~x_mask
        pooled_X = pooling(padded_cdr, epitope, combined_mask)
...
def validate_epoch(model, pooling, val_loader, plm_type, loss_func, wandb, device):
    ...
    for X, context, y in val_loader:
        ...
        batch_size = len(X)
        x_mask = torch.zeros((batch_size, batch_max), dtype=torch.bool)
        for i, seq in enumerate(X):
            for j, aa in enumerate(seq):
                if aa == 'X':
                    x_mask[i, j] = True
        ...
        val_loss += loss_func(preds, y.float()).item() * padded_cdr.shape[0]  #Use this line when the initial data contains gene names
